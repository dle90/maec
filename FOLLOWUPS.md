...
### Phase 1 — Data safety (Mongo hardening) — Units 1–6 of 9 shipped 2026-06-16/17
...
- **Unit 5** (`f99f8aa` + hotfix `2e04843`) — atomic FIFO `$gte` decrement (no
  oversell) + collapsed the 3 divergent fifoDeduct copies into `lib/fifoDeduct.js`
  + InventoryLot `strict:'throw'` + collision-safe auto-deduct phiếu numbers.
  `scripts/smoke-fifo-concurrent.js`. (Hotfix: Mongoose 9 needs `updatePipeline:true`.)
- **Unit 6** (`bd0502f` + foundation hotfix `32db5fd` + `d86a840`) — multi-doc
  **transactions** via `db.js` `withTxn` (topology-aware; awaits connection;
  never double-runs). 6a: encounter payment/checkout/refund (closes
  double-first-payment race). 6b: inventory confirm (+ paired transfer) +
  stocktake-approve. 6c: ris auto-deduct. `scripts/smoke-txn.js`. (Foundation
  hotfix: `supportsTransactions()` raced the connection → cached false → txns
  were silently inert; now awaits readiness + has a no-op-txn fallback probe.)
- **Remaining: Units 7–9** — audit plugin (createdBy/updatedBy via
  AsyncLocalStorage) (7), integer-VND money setters (8), `strict:'throw'` on
  catalog/inventory-master models after a `...req.body`→schema-whitelist cleanup
  (9, HIGH risk). See [docs/prod-upgrade-plan.md](docs/prod-upgrade-plan.md).
- **Deferred / fast-follows:** (a) `billing.js` pay/refund txn-wrap — confirm
  it's still live vs the `Encounter.payments[]` ledger first (no inventory; already
  hardened by Unit 3/4). (b) Hoist `Date.now()`/random ID generation outside the
  `withTxn` callbacks for retry-determinism (non-blocking; rollback is atomic so
  nothing orphaned commits today).
...
### Diagnostic — external validation + safety-gap fixes (2026-06-14)

Ran 22 documented EyeRounds/PMC cases through prod, root-caused every miss (8 genuine KB/engine gaps vs 5 mapping/metric artifacts), then fixed and re-validated. Live on master `630a246`; prod Atlas reseeded.

- **3 new red-flag rules** (`redFlags.json`): `rf-pdr` (neovascularization), `rf-rvo` (retinal hemorrhages + macular edema), `rf-wet-amd` (subretinal fluid + central vision symptoms). These three are flagged emergencies in `diseases.json` but previously had **no rule**, so could never fire.
- **4 over-strict triggers relaxed:** GCA now also fires on headache / elevated ESR-CRP (was jaw-claudication/scalp-tenderness only — the highest-consequence miss); retinal detachment fires on curtain **or** flashes **or** new floaters (was flashes AND floaters); bacterial keratitis dropped mandatory photophobia; optic neuritis accepts colour desaturation / pain-on-movement (catches bilateral MOG-ON). **Then tightened to stop over-fire:** RD no longer triggers on bare `field_loss` (shared with glaucoma/neuro — was firing on every POAG/NAION), ON no longer on bare `monocular` (was firing on keratitis).
- **Ranking edges:** `d-rvo↔intraretinal_fluid` (DME was outscoring RVO on macular edema), `d-wet-amd↔retinal_hemorrhages`, `d-hsv-keratitis↔pain_severe_or_moderate` (replaced the severe-only edge).
- **Engine — qualifier→tag normalization** (`orchestrator.js` `normalizeComplaint`): derive `monocular`/`binocular` from `eyeAffected` and `pain_severe`/`pain_severe_or_moderate` from the `pain` qualifier, so laterality + pain feed the gate and ranker even when the LLM parser only set the enum. Fixes the recurring "monocular not emitted" red-flag miss at the source.
- **KB-vocabulary guard** (`routes.js` + `findingExpansion.getKnownFindingIds`): the observations + complaint endpoints now **reject** any finding tag not in the KB vocabulary (HTTP 400). Previously a typo'd `findingId` (e.g. `periorbial_swelling`) stored silently and matched nothing — silently killing an emergency alert. Verified live on prod.
- **Scorecard (prod, pre→post):** top-1 55%→77%, top-3 82%→91%, intake-top3 32%→50%, red-flag recall 36%→93% (13/14; the one remaining miss is an atypical acute-on-chronic AACG case the extractor mapped as painless/gradual). Smoke stays 60/60; validate-kb clean (22 red-flags, 255 edges).

**Still deferred (genuine, not engine bugs):** the 2 remaining top-3 misses (macular hole, keratoconus) are **mapping artifacts** — the documented finding existed and the KB scores it strongly (`corneal_steepening_topo` es 0.95 for keratoconus; `metamorphopsia`/`central_scotoma` for macular hole) but wasn't extracted. Real fix is in the extraction layer (added VN colloquial aliases for metamorphopsia/central_scotoma; topography K-reading is a measurement, entered not parsed). All relaxed/new red-flag triggers + new edge weights still want a **clinical sign-off pass** ([CHECK]). Validation harness in `tmp-playwright/` (gitignored): `val-specs.json`, `val-local.cjs` (engine-direct vs Docker Mongo), `prod-probe.mjs` (deploy sentinel + prod scorecard).

### Diagnostic — validation round 2 (broad batch) + Tier-1 fixes (2026-06-14)

Second batch: **55 fresh documented cases across 46 diseases** (new diseases + atypical variants, diversified away from batch-1's 22) discovered/mapped by a 14-bucket subspecialty agent workflow, scored on prod, then adversarially root-caused with an explicit **overfit screen** (does each fix generalize or just make one case pass?). Live on master `d890820`; prod Atlas reseeded (24 red-flags / 261 edges).

- **Raw batch-2 prod scorecard:** top-1 30/55 (55%), top-3 44/55 (80%), intake-top3 39/55 (71%), red-flag 4/8. The intake-vs-final top-3 gap (71% vs 80%) localises ~9 pts of loss to the **symptom→finding mapper**, not the ranker — biggest untapped lever (audit deferred).
- **Tier-1 applied (generalizing, overfit-safe):** 3 safety rules — `rf-neovascular-glaucoma` (elevated_IOP+neovascularization), `rf-hyperacute-bacterial-conjunctivitis` (purulent discharge+periorbital swelling; swelling is the guardrail vs routine conj — verified no false fire), and `corneal_infiltrate` added to `rf-bacterial-keratitis`. Edges: keratoconus↔refractive_astigmatism, **myopia↔night_blindness refuting (-0.8)** (incidental myopia was burying RP), acanthamoeba↔corneal_infiltrate (→#1), NVG↔vision_drop, blepharitis↔discharge+periorbital_swelling.
- **Result:** batch-2 top-3 80%→85%, red-flag **4/8→7/8**; batch-1 unchanged (77%/91%/13-14); smoke 60/60.

**Tier-2 — coverage gaps (deferred, need new findings/diseases, a KB-authoring sprint):**
- **New findings:** `vitritis` (+ snowbanking/snowball/focal choroidal infiltrate) — unblocks posterior/intermediate uveitis (currently indistinguishable from anterior uveitis → mis-ranked + the posterior-uveitis red-flag can't be authored without it); `epithelial_defect`/`fluorescein_uptake` — the *correct* fix for corneal abrasion (do NOT add shared symptom edges — overfits); granular motility findings `abduction_deficit`/`vertical_gaze_restriction` — to separate CN6/CN4/CN3 palsies (currently cluster ~0.23-0.27).
- **New diseases (clinic-relevant first):** central serous retinopathy (CSR), Fuchs dystrophy, exposure keratopathy, paediatric strabismus (eso/exotropia), nasolacrimal duct obstruction; then Stargardt, myopic maculopathy, mac-tel, VMT, marginal keratitis, scleritis sub-types, congenital nystagmus.
- **Populate `d-rop` edges** (currently ZERO → can never rank; low priority, screening-driven).

**Discard pile (NOT engine bugs, do not tune):** ocular hypertension #4 (correct uncertainty — only elevated IOP, no disc/field data; boosting it would over-fire on early POAG); corneal-abrasion-via-`corneal_white_spot` (mapping artifact — wrong finding fed in). Batch-2 harness: `val-specs-2.json`, `score-batch.mjs` (parametric prod scorer + vocab-guard/typo reporter), `val-local.cjs <specs>` (engine-direct).

### Diagnostic — validation round 3 + Tier-2 coverage sprint + mapper audit (2026-06-14)

**Batch 3 (40 hard-discrimination cases):** covered the last 10 untested KB diseases → **71/71 KB diseases now validated** across 3 batches (117 cases). Stressed overlapping-presentation clusters (red eye, acute vision loss, diplopia/motility). Prod: top-1 68%, top-3 83%, red-flag 12/14. One Tier-1 fix shipped (`8104c87`): **`rf-occipital-stroke`** (fires on `vf_homonymous` — exclusive to occipital stroke, can't over-fire) → red-flag 12/14→14/14. Adversarial verify rejected the episcleritis/ERM edge-bumps as overfit (sent to Tier-2).

**Tier-2 coverage sprint (`2fe56f8`):** authored via a 10-area workflow + overfit screen, integrated, regression-gated against all 3 batches. **+17 findings, +5 diseases, +32 edges, +1 red-flag** → KB now 76 dz / 137 findings / 26 red-flags / 293 edges.
- New findings fix prior misses: directional motility (`diplopia_vertical/horizontal`, `head_tilt`, `abduction_deficit`) → CN4/CN6 separation (#1 now); `epithelial_defect` → corneal abrasion (#1); `vitritis` → posterior uveitis (#1) + `rf-posterior-uveitis`; `sectoral_redness_episclera` → episcleritis (had ZERO edges; #1 now); `cellophane_membrane_oct` → ERM (#1, wet-AMD unaffected).
- New diseases (all rank #1 except CSR ~#2): **d-csr, d-fuchs-dystrophy, d-nasolacrimal-duct-obstruction, d-strabismus-pediatric, d-exposure-keratopathy**. All carry a `[CHECK]` provenance flag for clinical sign-off. CSR shares `subretinal_fluid` with wet-AMD (top-3, not #1) — a known age-discrimination limit; would need an age-factor or refuting edge to fully separate (deferred).
- Tier-2 validation cases: prod top-3 **19/19 (100%)**. Regression: batches 1+3 unchanged, batch-2 final unchanged. Smoke 60/60.

**Mapper audit (`e11ccf3`):** 40 realistic Vietnamese free-text complaints scored against the **production `parseComplaint` LLM** (the extraction layer the pre-mapped validation batches never exercised). True quality (parseable findings, specificity-tolerant): **recall 97%, precision 78%, onset 70%** — the raw 75%/75% was depressed by the gold-set expecting `sign`/`test_result` tags the parser correctly doesn't emit. Real weakness = OVER-extraction (phantom `central_scotoma`/`near_only_blur` from vague "mờ") + under-committed onset. Fix = two generalizing `parseComplaint` prompt rules (don't over-specify vague blur; sharper VN onset cues). Re-measured with the real API: **precision 78%→83%, onset 70%→76%, recall held 97%.** `sudden` onset (red-flag-critical) is caught 16/18.

**Still deferred:** CSR-vs-wet-AMD age discrimination; the remaining over-extraction of `central_scotoma` (gold-set noise, not chased to avoid overfit); the 5 new Tier-2 diseases want clinical sign-off ([CHECK]); more coverage diseases noted but not added (Stargardt, myopic maculopathy, mac-tel, VMT, marginal keratitis, congenital nystagmus, scleritis sub-types). Round-3 harness: `val-specs-3.json`, `mapper-audit-set.json` (VN prose + gold), `score-mapper.mjs` (prod parser audit), `mapper-local.cjs` (direct parseComplaint test via `railway run`).

### Diagnostic — exam→engine wiring + routine-checkup ready-state (2026-06-14)

- **Routine check-up ready-state** (`40d66d3`): an asymptomatic visit yields a blank session (no symptoms/findings → empty differential). Previously the next-tests panel showed a misleading "đã có đủ thông tin" note; now a blank session shows a calm "🟢 Trợ lý sẵn sàng" banner instead of empty panels.
- **Exam → dx engine wiring** (`18cda4f`) — the incidental-finding safety net. New `diagnostic/examMapping.js` translates the encounter's NUMERIC exam outputs (`encounter.assignedServices[].output`, keys per `config/serviceOutputFields.js`: `od_icare`/`od_goldmann`→IOP, `od_sphere/cyl/axis/k1/k2`→refraction, `od_va_*`→VA with Snellen→decimal parse, `od_rnfl_avg`, `od_pachymetry`, `od_axial_length`, `od_mm`→Schirmer, `tbut_od`, `stereopsis`) into the engine's Shape-B measurement form; the engine derives the findings (IOP≥22→`elevated_IOP`, SE≤−0.5→`refractive_myopia`, RNFL<80→`rnfl_thinning`…). **Free-text fundus/slit-lamp notes (`od_findings`) are intentionally NOT mapped** — VN prose→finding is unreliable; the clinician enters those signs (or the LLM test-result parser does).
  - New endpoint `POST /diagnostic/sessions/:id/sync-exam {encounterId}`: maps + applies measurements (supersedes per-test rows → **idempotent on re-sync**), backfills `patientContext.ageYears/sex` from the encounter (dob/gender) for age-gated red-flags, re-runs, returns `{..session, syncedExam}`. Shared `applyMeasurementRows` helper (refactored out of the observations handler).
  - Khám embed: **auto-syncs once when the dx section is opened** (no session yet) + a "🔄 Lấy kết quả khám vào trợ lý" button to re-sync after more results; shows what was pulled.
...
### Khám — adversarial flow review + safety/authorization fixes (2026-06-15)

Multi-agent adversarial review of the consolidated Khám flow (clinical / UX / tech): 46 findings, 15 serious adversarially re-verified. Verdict: flow fundamentally sound (engine reasoning + 7 audited tech features work; "routine visits blocked" was a false alarm). Three fixes shipped + verified on prod:
- **Settled-encounter authorization sweep** (`d5fd93e`): a paid/cancelled encounter was still mutable. Added `isSettled()` 409 guard to 8 mutation endpoints (assign-package, service add/update/delete, clinical-notes, bill-item add/delete) + the diagnostic `/outcome` close (409 if already closed) + the Kham embed drops `serviceMode` (no order buttons) + "🔒 đã đóng" banner when closed. Verified: order on cancelled → 409.
- **Age-gated red-flag safety** (`d5fd93e`): `redFlagGate.matchPatientContext` now FAILS an age gate when age is missing (was passing) — stops GCA/NAION (≥50) or leukocoria (≤8) firing for the wrong age (e.g. a 25-yo migraine → GCA "IV steroids"). Verified: rf-gca fires only at age 68, not 25/missing; rf-rao (no age gate) still catches age-unknown emergencies. No batch regression (all age-gated validation cases carry a valid age).
- **Cycloplegic wait** (`f411669`): block finalizing the WET refraction (`SVC-CYCLO` status=done with wet values) until enough time since `drops_at`. Agent-aware: Tropicamide 25 / Cyclopentolate 40 / Atropine 0 (home-instilled) — clinically truer than a flat 45. Server returns 409 `CYCLO_WAIT`; client shows a live countdown + "💧 Nhỏ thuốc bây giờ" + disables done until ready. Recording drops_at (in_progress) always allowed. Verified all 4 cases on prod.
...
### Dx engine — POST /sessions crash-hardening + CSC coverage gap (2026-06-15)

**Incident + fix (`shipped`):** a complaint with an out-of-enum field crashed prod. `DxSession.complaint.eyeAffected` is `enum:['OD','OS','OU','unknown']`; a test payload sent `'right'` → `DxSession.create()` threw a Mongoose `ValidationError` inside `POST /api/diagnostic/sessions`, which had **no try/catch** → unhandled rejection → **Node process crash → 502 on every route** (crash-loop under Railway restart backoff). Fixed: (a) `POST /sessions` wrapped in try/catch → `ValidationError` returns **400 `INVALID_COMPLAINT`**, else 500; (b) `index.js` now has process-level `unhandledRejection`/`uncaughtException` handlers that log instead of letting Node exit — a single bad request can no longer take down the engine. Validated on prod: bad-enum → 400 + `GET /` stays 200. **Lesson: the other session mutation routes (`/observations`, `/sync-exam`, `/complaint`) also lack try/catch — the global guard now prevents a crash, but they should get explicit try/catch → 400 for clean errors (deferred).** Also: a few orphan `dx_*` sessions (no patient) were created on prod during this repro — harmless, can prune later.

**Symptom catalog expansion + picker (shipped `index-DKpSyti0`):** the dx intake chip list went from ~15 to the full **55** patient-reportable chips (all 54 `kind:'symptom'` findings + graded redness; the two pain-tier duplicates `pain_severe`/`pain_severe_or_moderate` stay folded into the graded "Đau" chip), grouped into **8 categories**. Built + 3-lens verified by the `symptom-catalog-audit` workflow (clinical / engine-contract / UX — overallOk on all; every red-flag trigger symptom pickable; every id a real finding `_id`; 2 verifier fixes applied — blanked over-asserted impliesVision/impliesOnset on vision_loss_subacute & field_loss_altitudinal, moved eyelid_twitch to Mí mắt). UI: inline chip wall replaced by a searchable, grouped **"+ Thêm triệu chứng"** dropdown (`SymptomPicker`, click-outside to close, multi-add). This closes the CSC gap — verified on prod engine: metamorphopsia → CSC #1 (0.476); + central_scotoma → 0.896. Also #2 done: the run button shows a "🔵 Có thay đổi chưa cập nhật" cue (+ ring) when inputs differ from the last run (`dirty` = input signature vs last-run signature).

**CSC nameVi is wrong (deferred — needs go-ahead + reseed):** `d-csr.nameVi` in diseases.json is **"Bệnh thấm thấu huyết tương dưới loạn"** — a garbled translation. Standard term is **"Bệnh hắc võng mạc trung tâm thanh dịch (CSC)"** (what the clinician expects). One-line KB edit + prod reseed (`railway ssh "node maec-app/server/diagnostic/seed.js"`). Worth a broader nameVi quality pass on the KB while at it.

**CSC coverage gap (FIXED via the catalog above):** test case "38M, painless monocular halos 3 ngày" → engine led with Glôcôm góc đóng cấp [emergency] 0.124, then RD + optic neuritis (all via `halos`/`monocular`); **Central serous (`d-csr`) scored zero**. Root cause: the patient's "quầng" was entered as the `halos` chip = "Halos around lights" (evokes acute angle-closure 0.85/0.85, no edge to CSC). CSC is evoked by **metamorphopsia / central_scotoma / micropsia** (edges already exist: edges.json 344-349, → also AMD/macular-hole/ERM) — but **none of those findings are in the UI symptom chip list** (`SYMPTOMS` in Diagnostic.jsx), so a macular presentation can't be captured via chips; only via free-text AI parse (aliases exist: "méo hình", "mờ vùng trung tâm") or exam OCT. **Proposed fix:** add chips "Méo hình" (metamorphopsia), "Mờ/ám điểm trung tâm" (central_scotoma), "Nhìn vật nhỏ" (micropsia) — UI + small mapping, no disease reseed.

**Pertinent-negative down-ranking (SHIPPED 2026-06-16, ranker.js):** the additive scorer never subtracted for explicit negatives, so an emergency could lead despite the patient explicitly lacking its hallmark (painless/non-red "halos" → acute angle-closure #1, though AACG = severe pain in 95%). Now: explicit `'none'` qualifiers (pain, visionChange) map to the finding tags they'd produce; for each NON-red-flag candidate the **symptom-derived** suspicion is dampened ×(1 − frequency·0.7) per absent *usual* finding (freq ≥ 0.6), surfaced in `refutingFindings`. **Critical design point** (a regression the validation batch caught): the penalty applies ONLY to symptom evocation, not observation-derived — `baseScore = symScore·refuteFactor + obsScore` — so a confirmatory exam sign (closed-angle gonioscopy) is never overridden by a history negative. Guardrails (verified on prod): explicit `'none'` only (never `unknown`); never touches a red-flag candidate; red-flag GATE untouched. Validation batch unchanged at **77/91/93** (zero net regression). **UI capture (SHIPPED 2026-06-16):** ComplaintForm now has a "Không có (đã hỏi): [Không đau] [Không đỏ] [Thị lực bình thường]" toggle row → sets the qualifier to `'none'` (positives always win; adding a conflicting symptom auto-clears+disables the toggle; AI parse also captures parsed `'none'`). So the down-rank fires in the normal chip flow. Verified end-to-end on prod (toggle "Không đau" on a halos complaint → acute angle-closure drops from the differential).

**Prod test-data prune (DONE 2026-06-16):** `scripts/cleanup-testdata.js` (dry-run default; `--apply` deletes; scoped to `_TEST*_`/`_ADVR_` patient names + their encounters + linked/open-orphan dx-sessions; **keeps closed/confirmed dx-sessions** = training signal). Ran on prod: deleted 14 patients, 17 encounters, 543 dx-sessions; 9 confirmed sessions preserved; 0 test rows left. Re-runnable for future cleanups.
...
- **Pane header slimmed** (follow-up, `index-DhGiR-CG build`): the encounter pane's own header went from 2-3 lines + 4 inline buttons to ONE line — identity (name · BN · site chip · status) left, occasional actions (In Phiếu Khám / Phiếu kết quả .docx / Đổi cơ sở / Hủy) in a `HeaderMenu` ⋯ dropdown, + ×. enc id → name `title` tooltip; paid/concluded detail dropped from the header (lives in the bill rail / Thu ngân). Also removed the embedded VN/EN toggle + "Phiên mới" row (VN-first; toggle stays on standalone /diagnostic). Verified `verify-top-prod.mjs`.
- **Empty red-flag grey bar + per-cell expand** (follow-up, `index-Dk3vZqO4 build`): (a) a blank encounter showed an empty grey strip under the pane header because the sticky banner wrapper rendered whenever `slots.redFlags` was a truthy element — but `RedFlagPanel` returns null with zero flags. Fixed by gating the wrapper on `slots.hasRedFlags` (`redFlags.length > 0`); shows only when closed / error / real flags. (b) Each `GridCell` now has a ⤢ toggle that lifts its `xl:max-h-[46vh]` cap so it grows to fit all content; the grid is `xl:items-start` so cells size independently and expanding one never balloons its row-mate. Verified `verify-greybar-expand.mjs` (grey bar gone; cell ① 368↔1127px).
- **Dx intake controls de-crowded** (follow-up, `index-Ci6qJPCo build`): section 3 (Tiền sử/bối cảnh) had used a VIEWPORT-based `lg:grid-cols-4`, which crammed 4 columns into the half-width cell ① at xl → the "Đeo kính tiếp xúc" tristate label wrapped over its pills. Fixed by making `TristateChip` a self-contained row (label truncates + tooltip, pills nowrap, `flex-1 min-w-[150px]`) inside a CONTAINER-based `flex-wrap` (packs 1–2 per row by real cell width). Section 2 symptom rows now group each labeled pill-set (Mắt/Khởi phát/Mức độ) as a nowrap unit so a narrow cell wraps between groups, not mid-group. Lesson: inside the 2×2 cells, prefer container-based `flex-wrap`/`min-w` over viewport `sm:`/`lg:grid-cols-*` (the cell width ≠ viewport width). Verified `verify-form-prod.mjs` at 233px.
