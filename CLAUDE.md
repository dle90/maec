...
- **Hosting**: Railway (Node app), MongoDB Atlas (database). **Cloudflare R2** (bucket `maec-attachments`, APAC) live for encounter + equipment file attachments ([lib/r2.js](maec-app/server/lib/r2.js)); credentials in Railway + local `.env`. Smoke probe: [scripts/smoke-r2.js](maec-app/server/scripts/smoke-r2.js).
...
- [ ] Document-path **watched-folder ingestor** (Medmont, AB800, IDRA) → auto-attach to encounter — manual PDF upload + Equipment module already live (Cloudflare R2; see FOLLOWUPS)
...
- [x] **Equipment module** ('Thiết bị' page) — shipped 2026-05-23. 16 devices seeded across 2 sites; 11 vendor contracts/quotes attached via R2. See FOLLOWUPS "Equipment module — shipped 2026-05-23".
